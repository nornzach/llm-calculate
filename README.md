...
- **缓存与显存**：支持共享前缀、FP8/FP4 KV、allocator 开销、KV offload、adapter/draft、MTP 和媒体 encoder；KV 压缩只在硬件与引擎都支持时应用。显存保护计入生成期间的 KV 增长，共享前缀仅作用于输入；模型上下文上限按输入加输出校验。
...
3. 查看模型矩阵：单元格只表示显存容量。显存余量与执行支持状态分别标注；`?` 表示执行未知或不支持，`—` 表示显存装不下，均不代表速度。
4. `no-acc` 表示缺少对应原生低精度算力路径。基础模型的量化是场景选择，预量化 checkpoint 保留原生权重格式。
...
矩阵的 `fit` 仅表示显存：`0` 超容量、`1` 余量不超过 10%、`2` 余量超过 10%。执行是否支持由 `support` 单独表示；未知或不支持的执行路径保留容量诊断和 `?` 标记，不输出速度，也不误标为显存不足。
