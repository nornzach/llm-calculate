...
### 线上旧目录覆盖问题（2026-09-05 追加）

- 用户提供 `https://calculate.nornnet.com/#rec` 后，读取线上 `/api/models`：Qwen3.8-27B 缺少 `heads`、`revision` 和 `param_source`；响应为 HTTP 200、Cloudflare DYNAMIC。线上页面内联 JS/CSS 与本地新版文件一致，说明界面更新并不能证明模型目录同步更新。
- 启动代码原先优先读取工作目录中的采集 JSON，旧文件会整份覆盖新版二进制内置目录。新增“只更新二进制、保留旧 JSON”的测试先复现失败，再修复加载顺序。默认使用内置数据；只有显式传入 `-data-dir` 才读取外部采集库，缺失或无效文件保留内置目录，并记录读取/解析错误。
- 用线上返回的完整 2257 条记录作为临时部署目录中的旧文件，启动新编译二进制：Qwen3.8-27B 返回 24 query heads、固定 revision 和 safetensors 参数来源；8192 输入、512 输出、16 并发、6000 TPM 的 `/api/recommend` 返回 HTTP 200 和 12 个 Pareto 候选。旧文件内容保持不变。
- **线上部署已验收（2026-09-05 23:43 CST）**：SSH 确认 systemd 服务以 `/opt/llm-calculate` 为工作目录，其中旧 JSON 缺少上述字段，源码目录中的记录则已更新。部署干净提交 `1efa85bffcab25074f7e5717c1c0c16d8b90271d` 构建的 Linux amd64 静态二进制，服务用户 `www-data` 的临时端口验证通过后替换并重启 `llm-calculate.service`；原二进制保留为 `llmcalc.rollback-20260905T154329Z`。新二进制 SHA-256：`3ddf102677201041f6bdae1834daf2f22ac5778d5ed9fe3b8c64ca72ab141a91`。
- 内部地址与公网域名的六个数据接口、推荐/规划/显存/性能接口均通过：默认 6000 TPM 返回 12 个 Pareto 候选及 12 个处方；NVFP4、1M TPM 返回 14 个方案。公网浏览器中，Qwen3.8-27B 地图恢复图表并显示 38 个有参考价候选，处方页显示 12 条；4 张 RTX 2080 的 70B Q4_K_M 仍正确显示显存不足。
- 服务器默认 `/usr/bin/go` 为 1.22.2，无法按当前模块要求自动取得 `go1.25`。本次使用 Go 1.25.3 交叉编译并核对上传前后哈希；服务器现有 `/usr/local/go/bin/go` 为 1.25.0，后续服务器构建应明确使用该路径。运行目录的旧 JSON 保留原样，当前服务默认读取新版内置目录。
...
- `go test -race -json ./...`：**121 项测试通过（含子测试）**，三个 Go package 全部通过；新增二进制更新回归及 HF/ModelScope 外部目录有效、缺失、JSON 损坏、模型记录无效的检查。
...
- 九个页面、详情和跨页动作的真实浏览器验证目标为 **http://localhost:8317/**。后续取得线上地址后的 API/页面核对及部署复现见上方追加记录；本地检查不代表线上已更新。
